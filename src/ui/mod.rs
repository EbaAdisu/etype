...
pub fn render(f: &mut Frame, app: &App) {
    match &app.screen {
        ...
        Screen::WordRush(state) => game::render_word_rush(f, state),
        Screen::Sentence(state) => game::render_sentence(f, state),
        Screen::Code(state) => game::render_code(f, state),
        Screen::Survival(state) => game::render_survival(f, state),
